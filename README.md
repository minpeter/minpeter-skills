...
| [`capturing-ui-screenshots`](skills/capturing-ui-screenshots/SKILL.md) | Reproducible Playwright evidence captures in Amp orbs: E2E/mock-auth builds, supervised preview services, precise API mocks, serial Radix Select and AlertDialog flows, visual verification, and cleanup. | A web-app change needs review screenshots or browser-state evidence, especially for animated overlays, detached locators, or an existing single-worker E2E flow. |
...
| [`tool-schema-design`](skills/tool-schema-design/SKILL.md) | Loss-aware design for LLM tool and function schemas: semantic source contracts, versioned provider-surface targets, wire-schema compilers, exact/reversible/lossy/unsupported diagnostics, runtime validation, required-versus-nullable semantics, and conformance cases. | Writing OpenAI, Anthropic, Gemini, Vertex, Bedrock, DeepSeek, Cohere, xAI, Fireworks, or MCP tool schemas; adapting Zod/Pydantic schemas; or reviewing portability claims across strict modes and schema dialects. |
